...
static matrix<pixel_data> read_bpm1(std::ifstream& input) {
	...
	for (uint64_t row = 0; row < raster.rows(); ++row) {
		...
		for (uint64_t col = 0; col < raster.cols(); ++col) {
			uint8_t palette_index = static_cast<uint8_t>(reader.read_value(1));
...
int main(int argc, char* argv[]) {
	...
	matrix<pixel_data> bpm = read_bpm1(input);
