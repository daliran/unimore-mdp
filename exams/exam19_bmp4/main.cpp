...
public:
	...
	uint8_t read_bit() {
		...
		uint8_t bit = buffer_ >> (bits_in_buffer_ - 1) & 1;
		bits_in_buffer_--;
		return bit;
	}

	uint64_t read_value(uint64_t bits_to_read) {
		uint64_t value = 0;
...
static matrix<pixel_data> read_bpm4(std::ifstream& input) {
	...
	for (uint64_t row = 0; row < raster.rows(); ++row) {
		...
		for (uint64_t col = 0; col < raster.cols(); ++col) {
			uint8_t palette_index = static_cast<uint8_t>(reader.read_value(4));
			palette_item color = palette[palette_index];
			raster(raster.rows() - 1 - row, col) = { color[0], color[1], color[2] };
		}

		// Ignore padding
		for (uint64_t i = 0; i < bits_of_paddings_per_row; ++i) {
			reader.read_bit();
		}
	}
...
int main(int argc, char* argv[]) {
	...
	matrix<pixel_data> bpm = read_bpm4(input);
