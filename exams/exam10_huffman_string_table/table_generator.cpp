...
int main() 
{
	std::ifstream is("data/bibbia.txt", std::ios::binary);
	...
	for (const auto& item : table) {
		os.write(reinterpret_cast<const char*>(&item._sym), sizeof(item._sym));
		os.write(reinterpret_cast<const char*>(&item._len), sizeof(item._len));
	}

	os.close();

	hufstr huffstr;
	auto compressed = huffstr.compress("I topi non avevano nipoti.");
